...
const QUOTA_PAYLOAD = {
  error: {
    ...
    details: [
      ...
      { "@type": "type.googleapis.com/google.rpc.RetryInfo", retryDelay: "2.456s" },
    ],
  },
};
...
describe("classifyGeminiError", () => {
  ...
  it("survives a message that is not JSON", () => {
    expect(() => classifyGeminiError(new Error("<html>502</html>"))).not.toThrow();
    expect(classifyGeminiError(new Error("<html>502</html>")).kind).toBe("other");
  });
