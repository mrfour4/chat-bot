...
export function AuthForm({ mode }: { mode: "signin" | "signup" }) {
  ...
  const [state, formAction] = useActionState(isSignUp ? signUp : signIn, EMPTY);

  return (
    <form action={formAction} className="space-y-4 rounded-lg border border-rule bg-white p-6">
      {isSignUp && (
        <Field label="Họ và tên" name="fullName" type="text" autoComplete="name" />
      )}
      ...
      {state.error && (
        <p role="alert" className="border-l-2 border-lacquer bg-lacquer-soft px-3 py-2 text-sm">
          {state.error}
        </p>
      )}
      {state.notice && (
        <p role="status" className="border-l-2 border-verified bg-panel px-3 py-2 text-sm">
          {state.notice}
        </p>
      )}
