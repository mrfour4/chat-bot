...
export async function GET() {
  ...
  try {
    ...
    if (error && error.code !== "42501") throw new Error(`${error.code}: ${error.message}`);
    checks.supabase = { ok: true, detail: "Connected, documents table reachable." };
  } catch (error) {
  ...
  if (env) {
    try {
      ...
      checks.gemini = { ok: true, detail: `API key valid. ${stores.length} File Search store(s).` };
      checks.fileSearchStore = env.fileSearchStore
        ...
        : { ok: false, detail: "Not configured. Run `npm run gemini:bootstrap`." };
    } catch (error) {
  ...
  return NextResponse.json({ ok, model: env?.geminiModel ?? null, checks }, { status: ok ? 200 : 503 });
}
